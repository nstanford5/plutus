...
lookup (P.toBuiltinData -> k) (Map d) = case lookup' k (BI.unsafeDataAsMap d) of
...
member (P.toBuiltinData -> k) (Map d) = member' k (BI.unsafeDataAsMap d)
...
insert (P.toBuiltinData -> k) (P.toBuiltinData -> a) (Map d) =
  unsafeFromBuiltinList (go (BI.unsafeDataAsMap d))
  where
    go ::
      BI.BuiltinList (BI.BuiltinPair BuiltinData BuiltinData) ->
      BI.BuiltinList (BI.BuiltinPair BuiltinData BuiltinData)
...
union ::
  ...
  Map k b ->
  Map k (These a b)
union (Map (BI.unsafeDataAsMap -> ls)) (Map (BI.unsafeDataAsMap -> rs)) = unsafeFromBuiltinList res
  where
...
unionWith ::
  ...
  Map k a ->
  Map k a
unionWith f (Map (BI.unsafeDataAsMap -> ls)) (Map (BI.unsafeDataAsMap -> rs)) =
...
toList (Map d) = go (BI.unsafeDataAsMap d)
  where
    go xs =
      P.matchList
        xs
